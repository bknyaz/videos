...
Fine tuning of the ResNet-50 network on the UCF101 video (action) classification dataset
...
from lasagne.layers import NonlinearityLayer, DenseLayer, DropoutLayer
...
parser = argparse.ArgumentParser(description='Fine tuning of the Resnet50 network on the video frames data')
parser.add_argument('-M','--model', default='/home/boris/Project/models/resnet/resnet50.pkl', help='location of the .pkl network')
parser.add_argument('-L','--layers', default='all', help='layers to be fine tuned (last, all)')
...
parser.add_argument('--save', default=20000, type=int, help='step after which save snapshots, 0 - do not save')
...
if args.layers != 'all':
    for layer in net.items():
        if (args.layers == 'last' and layer[0] == 'fc_ucf101'):
            continue
        try:
